...
class MoneyTransferProcessor(private val balanceRepository: BalanceRepository,
                             private val cmtRepository: CmtRepository) {

    @StreamListener(MoneyTransfer.CMT)
    @SendTo(MoneyTransfer.MTCO, MoneyTransfer.MTFA, MoneyTransfer.BACH)
    fun on(input: KStream<String, ConfirmMoneyTransfer>): Array<out KStream<String, SpecificRecord>>? {
        val isSucces = Predicate { _: String, v: SpecificRecord -> v is MoneyTransferConfirmed }
        val isFail = Predicate { _: String, v: SpecificRecord -> v is MoneyTransferFailed }
        val isBalance = Predicate { _: String, v: SpecificRecord -> v is BalanceChanged }

        return input
                .flatMap(this::getResponses)
                .branch(isSucces, isFail, isBalance)
    }
